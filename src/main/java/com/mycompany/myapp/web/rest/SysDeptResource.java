...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.SysDept}.
 */
@RestController
@RequestMapping("/api")
public class SysDeptResource {
    ...
    public ResponseEntity<SysDept> createSysDept(@RequestBody SysDept sysDept) throws URISyntaxException {
    ...
    public ResponseEntity<SysDept> updateSysDept(@RequestBody SysDept sysDept) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, sysDept.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<List<SysDept>> getAllSysDepts(Pageable pageable) {
        log.debug("REST request to get a page of SysDepts");
        Page<SysDept> page = sysDeptService.findAll(pageable);
    ...
    public ResponseEntity<SysDept> getSysDept(@PathVariable Long id) {
        ...
        Optional<SysDept> sysDept = sysDeptService.findOne(id);
